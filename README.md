...
這是一套繁體中文「體育預測內容後台＋會員點數＋推廣積分系統」，可部署到 GitHub Pages 作為白標 SaaS 展示站，也可串接 Supabase 作為會員、點數、內容、推廣與後台資料庫。

測試版 A1 已整合多種初版方向（內容轉換、推廣裂變、白標商業）為單一可開發版本。
...
- 虛擬商品兌換使用 `redeem_product` RPC，會同步寫入點數流水與訂單紀錄。
