...
[__atgSiteText(28), __atgSiteText(29)],
[__atgSiteText(30), __atgSiteText(31)],
[__atgSiteText(32), __atgSiteText(33)],
[__atgSiteText(34), __atgSiteText(35)],
[__atgSiteText(36), __atgSiteText(37)]
]);
const state = {
activeView: __atgSiteText(38),
bridgeReady: false,
bridgeConnected: false,
autoMonitorRequested: false,
snapshot: null,
notice: DEFAULT_NOTICE,
devices: {
channel: null,
items: [],
pendingPair: readPairParams(),
status: __atgSiteText(39)
},
filters: {
search: __atgSiteText(40),
minRank: 0,
minBurst: 0,
minRate: 0,
minBet: 0,
onlyEmpty: true,
sort: __atgSiteText(41)
},
cloud: {
canPublish: false,
channel: null,
...
publishTimer: null,
publishing: false,
status: __atgSiteText(42)
},
auth: {
busy: false,
client: null,
configured: false,
status: __atgSiteText(43),
user: null
},
portal: {
dbReady: false,
loading: false,
status: __atgSiteText(44),
profile: null,
tasks: [...DEFAULT_TASKS],
...
monitorControls: Array.from(document.querySelectorAll(__atgSiteText(95))),
monitorStatusGrid: byId(__atgSiteText(96)),
notice: byId(__atgSiteText(97)),
onlyEmptyInput: byId(__atgSiteText(98)),
openSourceBtn: byId(__atgSiteText(99)),
pointUnitInput: byId(__atgSiteText(100)),
refreshBtn: byId(__atgSiteText(101)),
rewardsList: byId(__atgSiteText(102)),
roomCount: byId(__atgSiteText(103)),
rooms: byId(__atgSiteText(104)),
saveSourceBtn: byId(__atgSiteText(105)),
sortMode: byId(__atgSiteText(106)),
sourceUrlInput: byId(__atgSiteText(107)),
signupBtn: byId(__atgSiteText(108)),
tasksList: byId(__atgSiteText(109)),
updatedAt: byId(__atgSiteText(110)),
userEmail: byId(__atgSiteText(111)),
viewTitle: byId(__atgSiteText(112))
};
const viewTitles = {
monitor: __atgSiteText(113),
modules: __atgSiteText(114),
account: __atgSiteText(115),
tasks: __atgSiteText(116),
redeem: __atgSiteText(117),
cards: __atgSiteText(118),
admin: __atgSiteText(119)
};
bindEvents();
initAuth();
render();
...
function bindEvents() {
window.addEventListener(__atgSiteText(120), handleWindowMessage);
els.authForm.addEventListener(__atgSiteText(121), (event) => {
event.preventDefault();
signIn();
});
els.signupBtn.addEventListener(__atgSiteText(122), signUp);
els.logoutBtn.addEventListener(__atgSiteText(123), signOut);
els.cloudBtn.addEventListener(__atgSiteText(124), toggleCloudPublishing);
els.connectBtn.addEventListener(__atgSiteText(125), connect);
els.monitorBtn.addEventListener(__atgSiteText(126), () => send(__atgSiteText(127)));
els.refreshBtn.addEventListener(__atgSiteText(128), () => send(__atgSiteText(129)));
els.pointUnitInput.addEventListener(__atgSiteText(130), updatePointUnit);
els.pointUnitInput.addEventListener(__atgSiteText(131), updatePointUnit);
els.saveSourceBtn.addEventListener(__atgSiteText(132), saveBoundUrl);
els.openSourceBtn.addEventListener(__atgSiteText(133), openBoundUrl);
els.checkinBtn.addEventListener(__atgSiteText(134), claimCheckin);
els.adminRefreshBtn.addEventListener(__atgSiteText(135), loadAdminWorkspace);
els.adminPublishTaskBtn.addEventListener(__atgSiteText(136), publishAdminTask);
els.adminPublishRewardBtn.addEventListener(__atgSiteText(137), publishAdminReward);
...
input.addEventListener(__atgSiteText(138), updateFiltersFromInputs);
input.addEventListener(__atgSiteText(139), updateFiltersFromInputs);
}
document.addEventListener(__atgSiteText(140), (event) => {
const nav = event.target.closest(__atgSiteText(141));
if (nav) {
setActiveView(nav.dataset.view);
return;
}
const enter = event.target.closest(__atgSiteText(142));
if (enter) {
enterRoom(enter.dataset.enterRoomId, enter.dataset.enterRoomNumber);
return;
}
const task = event.target.closest(__atgSiteText(143));
if (task) {
completeTask(task.dataset.taskId);
return;
}
const reward = event.target.closest(__atgSiteText(144));
if (reward) {
redeemReward(reward.dataset.rewardId);
return;
}
const adminSave = event.target.closest(__atgSiteText(145));
if (adminSave) {
saveAdminProfile(adminSave.dataset.adminSaveId);
return;
}
const adminToggleTask = event.target.closest(__atgSiteText(146));
if (adminToggleTask) {
setAdminTaskActive(adminToggleTask.dataset.adminToggleTask, adminToggleTask.dataset.active !== __atgSiteText(147));
return;
}
const adminToggleReward = event.target.closest(__atgSiteText(148));
if (adminToggleReward) {
setAdminRewardActive(adminToggleReward.dataset.adminToggleReward, adminToggleReward.dataset.active !== __atgSiteText(149));
}
});
...
if (message.type === __atgSiteText(150)) {
...
if (message.type === __atgSiteText(151)) {
...
state.notice = __atgSiteText(154);
} else if (message.type === __atgSiteText(155)) {
state.notice = message.payload && message.payload.message ? message.payload.message : __atgSiteText(156);
...
state.notice = state.snapshot && state.snapshot.lastMessage ? state.snapshot.lastMessage : __atgSiteText(158);
maybeAutoStartMonitoring();
scheduleCloudPublish();
} else if (message.type === __atgSiteText(159)) {
...
if (!window.supabase || typeof window.supabase.createClient !== __atgSiteText(161)) {
state.auth.configured = false;
state.auth.status = __atgSiteText(162);
renderAuth();
return;
}
...
state.auth.status = state.auth.user ? __atgSiteText(165) : __atgSiteText(166);
if (!state.auth.user) {
clearDashboard();
teardownCloud();
teardownDevices();
...
els.authPassword.value = __atgSiteText(168);
state.auth.status = __atgSiteText(169);
} catch (error) {
state.auth.status = formatAuthError(error);
} finally {
...
if (!credentials) return;
state.auth.busy = true;
state.auth.status = __atgSiteText(170);
renderAuth();
try {
...
if (error) throw error;
state.auth.user = data && data.session ? data.session.user : state.auth.user;
els.authPassword.value = __atgSiteText(171);
state.auth.status = data && data.session ? __atgSiteText(172) : __atgSiteText(173);
} catch (error) {
state.auth.status = formatAuthError(error);
} finally {
state.auth.busy = false;
...
state.auth.busy = true;
state.auth.status = __atgSiteText(174);
renderAuth();
try {
...
clearPortal();
clearAdmin();
state.auth.status = __atgSiteText(175);
} catch (error) {
state.auth.status = formatAuthError(error);
} finally {
state.auth.busy = false;
...
if (!state.bridgeConnected || !snapshot || !snapshot.connected) return;
...
state.notice = __atgSiteText(177);
send(__atgSiteText(178));
}
function getSnapshot() {
if (!isAuthenticated()) return;
send(__atgSiteText(179));
}
function send(type, payload = {}) {
if (!isAuthenticated()) return;
window.postMessage({ source: PAGE_SOURCE, type, ...payload }, __atgSiteText(180));
}
...
send(__atgSiteText(181), { settings: { pointUnit } });
}
async function setupDevices() {
if (!state.auth.client || !state.auth.user) return;
teardownDevices();
state.devices.status = state.devices.pendingPair ? __atgSiteText(182) : __atgSiteText(183);
renderDevices();
try {
...
.channel(__atgSiteText(184))
.on(
__atgSiteText(185),
{ event: __atgSiteText(186), schema: __atgSiteText(187), table: __atgSiteText(188) },
(payload) => {
...
.subscribe((status) => {
if (status === __atgSiteText(189)) {
state.devices.status = state.devices.items.length ? __atgSiteText(190) : __atgSiteText(191);
} else {
state.devices.status = __atgSiteText(192);
}
renderDevices();
});
} catch (error) {
state.devices.status = __atgSiteText(193);
renderDevices();
}
}
function teardownDevices() {
if (state.devices.channel && state.auth.client) {
state.auth.client.removeChannel(state.devices.channel);
}
state.devices.channel = null;
state.devices.items = [];
state.devices.status = state.devices.pendingPair ? __atgSiteText(194) : __atgSiteText(195);
}
async function claimPendingDevice() {
const pair = state.devices.pendingPair;
if (!pair || !state.auth.client) return;
const { data, error } = await state.auth.client.rpc(__atgSiteText(196), {
p_device_id: pair.deviceId,
p_pair_token: pair.pairToken
});
if (error) throw error;
state.devices.status = rpcMessage(data, __atgSiteText(197));
...
.from(__atgSiteText(198))
.select(__atgSiteText(199))
.order(__atgSiteText(200), { ascending: false, nullsFirst: false })
.limit(20);
if (error) throw error;
state.devices.items = data || [];
if (!state.devices.items.length) state.devices.status = __atgSiteText(201);
renderDevices();
}
async function loadLatestDeviceSnapshot() {
if (!state.auth.client) return;
const { data, error } = await state.auth.client
.from(__atgSiteText(202))
.select(__atgSiteText(203))
.order(__atgSiteText(204), { ascending: false })
.limit(1);
if (error) throw error;
const latest = data && data[0];
if (latest && latest.snapshot && !state.bridgeConnected) {
applyDeviceSnapshot(latest.snapshot, latest.updated_at, latest.device_id);
}
}
function applyDeviceSnapshot(snapshot, updatedAt, deviceId) {
...
? __atgSiteTpl(1,new Date(updatedAt).toLocaleTimeString(__atgSiteText(205), { hour12: false }))
: __atgSiteText(206);
render();
}
...
if (!state.auth.client || !state.auth.user) return;
teardownCloud();
state.cloud.status = __atgSiteText(207);
...
.from(__atgSiteText(208))
.select(__atgSiteText(209))
.eq(__atgSiteText(210), state.auth.user.id)
.maybeSingle();
if (broadcasterError && broadcasterError.code !== __atgSiteText(211)) throw broadcasterError;
...
.channel(__atgSiteText(213))
.on(
__atgSiteText(214),
{ event: __atgSiteText(215), schema: __atgSiteText(216), table: __atgSiteText(217), filter: __atgSiteText(218) },
(payload) => applyCloudSnapshot(payload.new && payload.new.snapshot, payload.new && payload.new.updated_at)
)
.subscribe((status) => {
state.cloud.status = status === __atgSiteText(219) ? (state.cloud.canPublish ? __atgSiteText(220) : __atgSiteText(221)) : __atgSiteText(222);
render();
});
render();
}
...
state.cloud.canPublish = false;
state.cloud.publishing = false;
state.cloud.status = __atgSiteText(223);
}
async function loadCloudSnapshot() {
if (!state.auth.client) return;
const { data, error } = await state.auth.client
.from(__atgSiteText(224))
.select(__atgSiteText(225))
.eq(__atgSiteText(226), __atgSiteText(227))
.maybeSingle();
if (error && error.code !== __atgSiteText(228)) {
state.cloud.status = __atgSiteText(229);
return;
}
...
state.notice = updatedAt ? __atgSiteTpl(2,new Date(updatedAt).toLocaleTimeString(__atgSiteText(230), { hour12: false })) : __atgSiteText(231);
render();
}
function toggleCloudPublishing() {
if (!state.cloud.canPublish) return;
state.cloud.publishing = !state.cloud.publishing;
state.cloud.status = state.cloud.publishing ? __atgSiteText(232) : __atgSiteText(233);
...
const { error } = await state.auth.client.from(__atgSiteText(234)).upsert(
{
id: __atgSiteText(235),
broadcaster_id: state.auth.user.id,
snapshot,
updated_at: new Date().toISOString()
},
{ onConflict: __atgSiteText(236) }
);
if (error) {
state.cloud.status = __atgSiteText(237);
} else {
state.cloud.lastPublishedAt = Date.now();
state.cloud.status = __atgSiteText(238);
}
render();
}
...
state.portal.status = __atgSiteText(241);
} catch (error) {
state.portal.dbReady = false;
state.portal.status = __atgSiteText(242);
} finally {
state.portal.loading = false;
...
const displayName = (user.email || __atgSiteText(243)).split(__atgSiteText(244))[0];
const { error } = await state.auth.client.from(__atgSiteText(245)).upsert(
{
id: user.id,
email: user.email || __atgSiteText(246),
display_name: displayName
},
{ onConflict: __atgSiteText(247) }
);
if (error) throw error;
}
async function loadProfile() {
const { data, error } = await state.auth.client
.from(__atgSiteText(248))
.select(__atgSiteText(249))
.eq(__atgSiteText(250), state.auth.user.id)
.maybeSingle();
if (error && error.code !== __atgSiteText(251)) throw error;
...
const { data, error } = await state.auth.client
.from(__atgSiteText(252))
.select(__atgSiteText(253))
.eq(__atgSiteText(254), true)
.order(__atgSiteText(255), { ascending: true });
if (error) throw error;
state.portal.tasks = data && data.length ? data : [...DEFAULT_TASKS];
}
async function loadRewards() {
const { data, error } = await state.auth.client
.from(__atgSiteText(256))
.select(__atgSiteText(257))
.eq(__atgSiteText(258), true)
.order(__atgSiteText(259), { ascending: true });
if (error) throw error;
state.portal.rewards = data && data.length ? data : [...DEFAULT_REWARDS];
}
async function loadLedger() {
const { data, error } = await state.auth.client
.from(__atgSiteText(260))
.select(__atgSiteText(261))
.eq(__atgSiteText(262), state.auth.user.id)
.order(__atgSiteText(263), { ascending: false })
.limit(30);
if (error) throw error;
...
const { data, error } = await state.auth.client
.from(__atgSiteText(264))
.select(__atgSiteText(265))
.eq(__atgSiteText(266), state.auth.user.id)
.order(__atgSiteText(267), { ascending: false })
.limit(50);
if (error) throw error;
...
const { data, error } = await state.auth.client
.from(__atgSiteText(268))
.select(__atgSiteText(269))
.eq(__atgSiteText(270), state.auth.user.id);
if (error) throw error;
...
state.admin.status = __atgSiteText(272);
renderAdmin();
try {
await Promise.all([loadAdminProfiles(), loadAdminTasks(), loadAdminRewards()]);
state.admin.status = __atgSiteText(273);
} catch (error) {
...
const { data, error } = await state.auth.client.rpc(__atgSiteText(275));
if (error) throw error;
state.admin.tasks = Array.isArray(data) ? data : [];
}
async function loadAdminRewards() {
const { data, error } = await state.auth.client.rpc(__atgSiteText(276));
if (error) throw error;
state.admin.rewards = Array.isArray(data) ? data : [];
}
async function saveAdminProfile(userId) {
if (!state.auth.client || !canUseAdminPanel()) {
state.admin.status = __atgSiteText(277);
renderAdmin();
return;
}
const row = adminRowById(userId);
if (!row) return;
const role = row.querySelector(__atgSiteText(278)).value;
const badge = row.querySelector(__atgSiteText(279)).value.trim();
const points = Math.max(0, Math.round(number(row.querySelector(__atgSiteText(280)).value)));
state.admin.loading = true;
state.admin.status = __atgSiteText(281);
renderAdmin();
try {
const { data, error } = await state.auth.client.rpc(__atgSiteText(282), {
p_user_id: userId,
p_role: role,
p_badge: badge,
p_points: points
...
if (error) throw error;
state.admin.status = rpcMessage(data, __atgSiteText(283));
await loadProfile();
await loadAdminWorkspace();
} catch (error) {
...
const title = [prefix, rawTitle].filter(Boolean).join(__atgSiteText(284));
...
const url = normalizeUrl(els.adminTaskUrl.value.trim()) || __atgSiteText(285);
if (!rawTitle) {
state.admin.status = __atgSiteText(286);
renderAdmin();
return;
}
state.admin.loading = true;
state.admin.status = __atgSiteText(287);
renderAdmin();
try {
const { data, error } = await state.auth.client.rpc(__atgSiteText(288), {
p_task_id: null,
p_title: title,
...
if (error) throw error;
state.admin.status = rpcMessage(data, __atgSiteText(289));
clearAdminTaskForm();
await loadTasks();
await loadAdminWorkspace();
} catch (error) {
...
if (!title) {
state.admin.status = __atgSiteText(290);
renderAdmin();
return;
}
state.admin.loading = true;
state.admin.status = __atgSiteText(291);
renderAdmin();
try {
const { data, error } = await state.auth.client.rpc(__atgSiteText(292), {
p_reward_id: null,
p_title: title,
p_description: description,
p_cost_points: cost,
...
if (error) throw error;
state.admin.status = rpcMessage(data, __atgSiteText(293));
...
if (!state.auth.client || !canUseAdminPanel()) return;
state.admin.loading = true;
state.admin.status = active ? __atgSiteText(294) : __atgSiteText(295);
renderAdmin();
try {
const { data, error } = await state.auth.client.rpc(__atgSiteText(296), {
p_task_id: taskId,
p_active: active
});
if (error) throw error;
state.admin.status = rpcMessage(data, active ? __atgSiteText(297) : __atgSiteText(298));
await loadTasks();
await loadAdminWorkspace();
} catch (error) {
...
if (!state.auth.client || !canUseAdminPanel()) return;
state.admin.loading = true;
state.admin.status = active ? __atgSiteText(299) : __atgSiteText(300);
renderAdmin();
try {
const { data, error } = await state.auth.client.rpc(__atgSiteText(301), {
p_reward_id: rewardId,
p_active: active
});
if (error) throw error;
state.admin.status = rpcMessage(data, active ? __atgSiteText(302) : __atgSiteText(303));
await loadRewards();
await loadAdminWorkspace();
} catch (error) {
...
state.portal.status = __atgSiteText(304);
renderPortal();
return;
}
if (state.portal.dbReady && state.auth.client) {
const { data, error } = await state.auth.client.rpc(__atgSiteText(305));
if (error) {
state.portal.status = formatAuthError(error);
renderPortal();
return;
}
state.portal.status = rpcMessage(data, __atgSiteText(306));
await loadPortal();
return;
}
addLocalPoints(20, __atgSiteText(307));
state.portal.profile.last_checkin_at = new Date().toISOString();
state.portal.status = __atgSiteText(308);
persistLocalFromState();
renderPortal();
}
...
if (!task) return;
if (task.url) window.open(task.url, __atgSiteText(309), __atgSiteText(310));
if (state.portal.completedTasks.has(taskId)) {
state.portal.status = __atgSiteText(311);
renderPortal();
return;
}
if (state.portal.dbReady && state.auth.client) {
const { data, error } = await state.auth.client.rpc(__atgSiteText(312), { p_task_id: taskId });
if (error) {
state.portal.status = formatAuthError(error);
renderPortal();
return;
}
state.portal.status = rpcMessage(data, __atgSiteText(313));
await loadPortal();
return;
}
state.portal.completedTasks.add(taskId);
addLocalPoints(number(task.reward_points), __atgSiteTpl(9,task.title));
state.portal.status = __atgSiteText(314);
persistLocalFromState();
renderPortal();
}
...
state.portal.status = __atgSiteText(315);
renderPortal();
return;
}
if (state.portal.dbReady && state.auth.client) {
const { data, error } = await state.auth.client.rpc(__atgSiteText(316), { p_reward_id: rewardId });
if (error) {
state.portal.status = formatAuthError(error);
renderPortal();
return;
}
state.portal.status = rpcMessage(data, __atgSiteText(317));
await loadPortal();
return;
}
...
if (reward.id === __atgSiteText(318)) state.portal.profile.badge = __atgSiteText(319);
...
if (!url) {
state.portal.status = __atgSiteText(320);
renderPortal();
return;
}
state.portal.profile = {
...
.from(__atgSiteText(321))
.update({ bound_game_url: url })
.eq(__atgSiteText(322), state.auth.user.id);
if (error) state.portal.status = __atgSiteText(323);
else state.portal.status = __atgSiteText(324);
} else {
...
const url = normalizeUrl(els.sourceUrlInput.value.trim() || (state.portal.profile && state.portal.profile.bound_game_url) || __atgSiteText(326));
if (!url) {
state.portal.status = __atgSiteText(327);
renderPortal();
return;
}
window.open(url, __atgSiteText(328), __atgSiteText(329));
}
function updateSnapshotFromExternalBridge(payload) {
if (!isAuthenticated()) return;
try {
const data = typeof payload === __atgSiteText(330) ? JSON.parse(payload) : payload;
const snapshot = data && data.snapshot ? data.snapshot : data;
if (!snapshot || !Array.isArray(snapshot.rooms)) {
state.notice = __atgSiteText(331);
render();
return;
}
state.snapshot = {
source: __atgSiteText(332),
ts: Date.now(),
connected: true,
monitoring: true,
scanning: false,
detailsScanning: false,
lastMessage: __atgSiteText(333),
...
state.filters.sort = els.sortMode.value || __atgSiteText(334);
renderRooms();
}
function setActiveView(view) {
if (!viewTitles[view]) return;
if (view === __atgSiteText(335) && !canUseAdminPanel()) {
state.portal.status = __atgSiteText(336);
view = __atgSiteText(337);
}
state.activeView = view;
for (const section of document.querySelectorAll(__atgSiteText(338))) {
section.classList.toggle(__atgSiteText(339), section.id === __atgSiteTpl(13,view));
}
for (const button of document.querySelectorAll(__atgSiteText(340))) {
button.classList.toggle(__atgSiteText(341), button.dataset.view === view);
}
...
state.notice = __atgSiteText(342);
render();
return;
}
send(__atgSiteText(343), { roomId });
...
document.body.classList.toggle(__atgSiteText(344), signedIn);
els.authStatus.textContent = state.auth.status;
els.userEmail.textContent = signedIn ? state.auth.user.email || __atgSiteText(345) : __atgSiteText(346);
...
els.bridgeStatus.textContent = state.bridgeConnected ? __atgSiteText(347) : state.bridgeReady ? __atgSiteText(348) : __atgSiteText(349);
if (snapshot && snapshot.cloudMode && !state.bridgeConnected) els.bridgeStatus.textContent = __atgSiteText(350);
if (snapshot && snapshot.source === __atgSiteText(351) && !state.bridgeConnected) els.bridgeStatus.textContent = __atgSiteText(352);
els.gameStatus.textContent = snapshot && snapshot.connected ? snapshot.lastMessage || __atgSiteText(353) : __atgSiteText(354);
els.roomCount.textContent = String(snapshot ? snapshot.totalRooms || rooms.length : 0);
els.updatedAt.textContent = snapshot && snapshot.ts ? new Date(snapshot.ts).toLocaleTimeString(__atgSiteText(355), { hour12: false }) : __atgSiteText(356);
...
const profileUrl = state.portal.profile && state.portal.profile.bound_game_url ? state.portal.profile.bound_game_url : __atgSiteText(357);
if (document.activeElement !== els.sourceUrlInput) {
els.sourceUrlInput.value = profileUrl || (snapshot && snapshot.url) || __atgSiteText(358);
}
}
function renderViewScope() {
const monitorVisible = state.activeView === __atgSiteText(359);
for (const element of els.monitorControls) {
element.hidden = !monitorVisible;
}
if (els.monitorStatusGrid) els.monitorStatusGrid.hidden = !monitorVisible;
if (els.notice) els.notice.hidden = !monitorVisible;
}
...
els.cloudBtn.textContent = state.cloud.canPublish ? (state.cloud.publishing ? __atgSiteText(360) : __atgSiteText(361)) : __atgSiteText(362);
}
function renderDevices() {
if (!els.devicePairStatus || !els.deviceList) return;
const pending = state.devices.pendingPair;
els.devicePairStatus.textContent = pending ? __atgSiteText(363) : state.devices.status;
if (!state.devices.items.length) {
els.deviceList.innerHTML = __atgSiteText(364);
return;
}
els.deviceList.innerHTML = state.devices.items
.map((device) => __atgSiteTpl(16,escapeHtml(device.label || shortDeviceId(device.id)),escapeHtml(device.status || __atgSiteText(365)),device.last_seen_at ? __atgSiteTpl(15,escapeHtml(formatDateTime(device.last_seen_at))) : __atgSiteText(366)))
.join(__atgSiteText(367));
}
function renderRooms() {
const snapshot = state.snapshot;
const rooms = filteredRooms();
if (!snapshot || !Array.isArray(snapshot.rooms) || !snapshot.rooms.length) {
els.rooms.innerHTML = __atgSiteTpl(17,escapeHtml(snapshot && snapshot.connected ? __atgSiteText(368) : __atgSiteText(369)));
return;
}
if (!rooms.length) {
...
els.rooms.innerHTML = rooms.map((room) => renderRoom(room, { localBridge: state.bridgeConnected })).join(__atgSiteText(370));
}
function renderPortal() {
const profile = state.portal.profile || fallbackProfile();
const points = number(profile.points);
const badge = profile.badge || __atgSiteText(371);
...
els.boundUrlText.textContent = profile.bound_game_url || __atgSiteText(372);
...
if (!els.adminNav || !els.adminList || !els.adminStatus) return;
const enabled = canUseAdminPanel();
els.adminNav.hidden = !enabled;
if (!enabled) {
if (state.activeView === __atgSiteText(373)) setActiveView(__atgSiteText(374));
els.adminRefreshBtn.disabled = true;
els.adminPublishTaskBtn.disabled = true;
els.adminPublishRewardBtn.disabled = true;
els.adminStatus.textContent = __atgSiteText(375);
els.adminList.innerHTML = __atgSiteText(376);
els.adminTaskList.innerHTML = __atgSiteText(377);
els.adminRewardList.innerHTML = __atgSiteText(378);
return;
}
...
? state.admin.profiles.map(renderAdminProfile).join(__atgSiteText(379))
: __atgSiteTpl(22);
els.adminTaskList.innerHTML = state.admin.tasks.length
? state.admin.tasks.map(renderAdminTask).join(__atgSiteText(380))
: __atgSiteTpl(23);
els.adminRewardList.innerHTML = state.admin.rewards.length
? state.admin.rewards.map(renderAdminReward).join(__atgSiteText(381))
: __atgSiteTpl(24);
}
...
.map(([value, label]) => __atgSiteTpl(25,value,value === role ? __atgSiteText(382) : __atgSiteText(383),label))
.join(__atgSiteText(384));
return __atgSiteTpl(26,escapeHtml(profile.id),escapeHtml(profile.email || profile.display_name || profile.id),escapeHtml(profile.display_name || __atgSiteText(385)),escapeHtml(formatRoleLabel(role)),escapeHtml(formatDateTime(profile.created_at)),isSelf ? __atgSiteText(386) : __atgSiteText(387),roleOptions,escapeHtml(profile.badge || __atgSiteText(388)),Math.max(0, Math.round(number(profile.points))),escapeHtml(profile.id),state.admin.loading ? __atgSiteText(389) : __atgSiteText(390));
}
function renderAdminTask(task) {
const active = Boolean(task.active);
return __atgSiteTpl(27,escapeHtml(task.title || task.id),formatNumber(task.reward_points),formatNumber(task.sort_order),escapeHtml(task.description || __atgSiteText(391)),active ? __atgSiteText(392) : __atgSiteText(393),escapeHtml(task.id),String(active),state.admin.loading ? __atgSiteText(394) : __atgSiteText(395),active ? __atgSiteText(396) : __atgSiteText(397));
}
function renderAdminReward(reward) {
const active = Boolean(reward.active);
return __atgSiteTpl(28,escapeHtml(reward.title || reward.id),formatNumber(reward.cost_points),formatNumber(reward.sort_order),escapeHtml(reward.description || __atgSiteText(398)),active ? __atgSiteText(399) : __atgSiteText(400),escapeHtml(reward.id),String(active),state.admin.loading ? __atgSiteText(401) : __atgSiteText(402),active ? __atgSiteText(403) : __atgSiteText(404));
}
...
return __atgSiteTpl(30,escapeHtml(item.action || __atgSiteText(405)),formatDateTime(item.created_at),amount >= 0 ? __atgSiteText(406) : __atgSiteText(407),amount >= 0 ? __atgSiteText(408) : __atgSiteText(409),formatNumber(amount));
})
.join(__atgSiteText(410));
}
...
return __atgSiteTpl(31,formatNumber(task.reward_points),escapeHtml(task.title),escapeHtml(task.description || __atgSiteText(411)),escapeHtml(task.id),done ? __atgSiteText(412) : __atgSiteText(413),done ? __atgSiteText(414) : __atgSiteText(415));
})
.join(__atgSiteText(416));
}
function renderRewards() {
const rewards = state.portal.rewards || [];
els.rewardsList.innerHTML = rewards
.map((reward) => __atgSiteTpl(32,formatNumber(reward.cost_points),escapeHtml(reward.title),escapeHtml(reward.description || __atgSiteText(417)),escapeHtml(reward.id)))
...
.map((order) => __atgSiteTpl(34,escapeHtml(order.reward_title || __atgSiteText(419)),formatDateTime(order.created_at),formatNumber(order.cost_points),escapeHtml(order.card_number || __atgSiteText(420))))
.join(__atgSiteText(421));
}
...
const statusClass = room.status === __atgSiteText(422) ? __atgSiteText(423) : __atgSiteText(424);
const tag = [room.tier, ...(Array.isArray(room.tags) ? room.tags : [])].filter(Boolean).join(__atgSiteText(425)) || __atgSiteText(426);
...
return __atgSiteTpl(37,escapeHtml(room.number || room.roomId || __atgSiteText(427)),escapeHtml(tag),rank.toFixed(0),burst.toFixed(0),burst.toFixed(1),burst.toFixed(0),formatRate(room.rate),formatNumber(room.bet),statusClass,escapeHtml(room.statusLabel || STATUS_LABELS[room.status] || room.status || __atgSiteText(428)),formatNullableRate(room.todayRate),formatNullableRate(room.hourRate),formatNullableRate(room.dayRate),formatTurns(room.notOpenTurns),formatTurns(room.previousOneTurns),formatTurns(room.previousTwoTurns),formatPointRange(room),formatPointValue(room.pointAvgPerTurn),escapeHtml(room.pointSource || __atgSiteText(429)),formatTurns(room.pointRemainingTurns),escapeHtml(room.pointStatus || __atgSiteText(430)),formatNullablePercent(room.pointConfidence),formatTurns(room.freeGameTargetTurn),formatTurnWindow(room),formatRemainingWindow(room),escapeHtml(room.freeGameForecastZone || __atgSiteText(431)),formatNullablePercent(room.freeGameForecastConfidence),formatNullablePercent(room.momentumScore),formatNullablePercent(room.cyclePressure),formatNullablePercent(room.pullbackRisk),formatNullablePercent(room.pointReadiness),formatPointValue(room.pointSampleShortfall),formatPointValue(room.pointSampleTarget),reasons.length ? __atgSiteTpl(36,reasons.map((reason) => __atgSiteTpl(35,escapeHtml(reason))).join(__atgSiteText(432))) : __atgSiteText(433),escapeHtml(room.roomId || __atgSiteText(434)),escapeHtml(room.number || __atgSiteText(435)),canEnter ? __atgSiteText(436) : __atgSiteText(437));
}
...
const haystack = __atgSiteTpl(38,room.number || __atgSiteText(438),room.roomId || __atgSiteText(439)).toLowerCase();
if (filters.search && !haystack.includes(filters.search)) return false;
if (filters.onlyEmpty && room.status !== __atgSiteText(440)) return false;
...
els.rooms.innerHTML = __atgSiteText(444);
renderCloudControls();
}
...
state.portal.status = __atgSiteText(445);
}
function clearAdmin() {
state.admin.loading = false;
state.admin.status = __atgSiteText(446);
state.admin.profiles = [];
state.admin.tasks = [];
state.admin.rewards = [];
}
function clearAdminTaskForm() {
els.adminTaskTitle.value = __atgSiteText(447);
els.adminTaskDesc.value = __atgSiteText(448);
els.adminTaskUrl.value = __atgSiteText(449);
els.adminTaskReward.value = __atgSiteText(450);
els.adminTaskSort.value = __atgSiteText(451);
}
function clearAdminRewardForm() {
els.adminRewardTitle.value = __atgSiteText(452);
els.adminRewardDesc.value = __atgSiteText(453);
els.adminRewardCost.value = __atgSiteText(454);
els.adminRewardSort.value = __atgSiteText(455);
}
function fallbackProfile() {
const user = state.auth.user || {};
return {
id: user.id || __atgSiteText(456),
email: user.email || __atgSiteText(457),
display_name: user.email ? user.email.split(__atgSiteText(458))[0] : __atgSiteText(459),
points: 100,
badge: __atgSiteText(460),
role: __atgSiteText(461),
bound_game_url: __atgSiteText(462),
...
return found ? found[1] : __atgSiteText(463);
}
function normalizeRole(role) {
const value = String(role || __atgSiteText(464)).trim().toLowerCase();
if (value === __atgSiteText(465) || value === __atgSiteText(466) || value === __atgSiteText(467) || value === __atgSiteText(468)) return __atgSiteText(469);
if (value === __atgSiteText(470) || value === __atgSiteText(471)) return __atgSiteText(472);
if (value === __atgSiteText(473) || value === __atgSiteText(474) || value === __atgSiteText(475)) return __atgSiteText(476);
if (value === __atgSiteText(477)) return __atgSiteText(478);
return __atgSiteText(479);
}
function canUseAdminPanel() {
return Boolean(state.portal.dbReady && state.portal.profile && normalizeRole(state.portal.profile.role) === __atgSiteText(480));
}
function adminRowById(userId) {
return Array.from(document.querySelectorAll(__atgSiteText(481))).find((row) => row.dataset.adminRowId === userId) || null;
}
function readLocalPortal() {
try {
return JSON.parse(localStorage.getItem(localPortalKey()) || __atgSiteText(482));
} catch {
return {};
}
}
...
const user = state.auth.user;
return __atgSiteTpl(39,user ? user.id || user.email : __atgSiteText(483));
}
...
if (value && typeof value === __atgSiteText(484)) {
...
state.auth.status = state.auth.configured ? __atgSiteText(486) : __atgSiteText(487);
renderAuth();
return false;
}
...
if (!email || !password) {
state.auth.status = __atgSiteText(488);
renderAuth();
return null;
}
if (password.length < 6) {
state.auth.status = __atgSiteText(489);
renderAuth();
return null;
}
...
if (!config || typeof config.url !== __atgSiteText(490) || typeof config.anonKey !== __atgSiteText(491)) return false;
if (!/^https:\/\/[a-z0-9-]+\.supabase\.co$/i.test(config.url.trim())) return false;
if (config.url.includes(__atgSiteText(492)) || config.anonKey.includes(__atgSiteText(493))) return false;
return config.anonKey.trim().length > 40;
}
function formatAuthError(error) {
const message = error && error.message ? error.message : String(error || __atgSiteText(494));
const normalized = message.toLowerCase();
if (normalized.includes(__atgSiteText(495))) return __atgSiteText(496);
if (normalized.includes(__atgSiteText(497))) return __atgSiteText(498);
if (normalized.includes(__atgSiteText(499))) return __atgSiteText(500);
if (normalized.includes(__atgSiteText(501))) return __atgSiteText(502);
if (normalized.includes(__atgSiteText(503))) return __atgSiteText(504);
return message;
}
function normalizeUrl(value) {
if (!value) return __atgSiteText(505);
const url = value.startsWith(__atgSiteText(506)) || value.startsWith(__atgSiteText(507)) ? value : __atgSiteTpl(41,value);
try {
return new URL(url).href;
} catch {
return __atgSiteText(508);
}
}
function readPairParams() {
try {
const params = new URLSearchParams(window.location.search);
const deviceId = params.get(__atgSiteText(509)) || params.get(__atgSiteText(510));
const pairToken = params.get(__atgSiteText(511)) || params.get(__atgSiteText(512));
...
for (const key of [__atgSiteText(513), __atgSiteText(514), __atgSiteText(515), __atgSiteText(516)]) {
...
const text = String(value || __atgSiteText(517));
if (text.length <= 12) return text || __atgSiteText(518);
...
return value == null ? __atgSiteText(519) : formatRate(value);
}
function formatNullablePercent(value) {
return value == null ? __atgSiteText(520) : __atgSiteTpl(45,clamp(number(value), 0, 100).toFixed(0));
}
function formatNumber(value) {
return new Intl.NumberFormat(__atgSiteText(521), { maximumFractionDigits: 2 }).format(number(value));
}
function formatTurns(value) {
return value == null ? __atgSiteText(522) : __atgSiteTpl(46,Math.round(number(value)));
}
function formatTurnWindow(room) {
if (room.freeGameWindowStart == null || room.freeGameWindowEnd == null) return __atgSiteText(523);
...
if (max <= 0) return __atgSiteText(529);
if (min === max) return formatPointValue(max);
return __atgSiteTpl(50,formatPointValue(min),formatPointValue(max));
}
function formatDateTime(value) {
if (!value) return __atgSiteText(530);
const date = new Date(value);
if (Number.isNaN(date.getTime())) return String(value);
return date.toLocaleString(__atgSiteText(531), { hour12: false });
}
function escapeHtml(value) {
return String(value)
.replace(/&/g, __atgSiteText(532))
.replace(/</g, __atgSiteText(533))
.replace(/>/g, __atgSiteText(534))
.replace(/"/g, __atgSiteText(535))
.replace(/'/g, __atgSiteText(536));
}
